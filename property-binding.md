...
![lab-icon](https://raw.githubusercontent.com/ngHeroes/todo-list-tutorial/master/assets/lab.jpg) **Playground**: For example, let's bind the input value to a method call that returns a value. First, let's add the method `generateTitle` anywhere inside the class, but not inside any of its methods.
...
![lab-icon](https://raw.githubusercontent.com/ngHeroes/todo-list-tutorial/master/assets/lab.jpg) **Playground**: To show this, let's change the value of the title after a few seconds and see what happens. Call the `setTimeout` function inside `ngOnInit`:
